...
class NerProcessor(DataProcessor):
    ...
    #     self._label_list = ['[PAD]', 'O', 'B-symptom', 'I-symptom', 'E-symptom', 'S-symptom', 
...
class BERT_CRF_NER(nn.Module):
    ...
    def __init__(self, bert_model, tag2idx, max_seq_length, device):
...
# ### 具体查看 CRF 层
# bert_seg_out, _ = model.bert(input_ids, token_type_ids=segment_ids, attention_mask=input_mask, output_all_encoded_layers=False)
# print(bert_seg_out.shape)
...
model = BERT_CRF_NER(bert_model, tag2idx, args.max_seq_length, device)
...
if args.do_train:
    ...
    torch.save({'epoch': num_epoch, 'model_state': model.state_dict(), 
            ...
            os.path.join(args.output_dir, 'bert_crf.ckpt'))
...
# Load a trained model and config that you have fine-tuned
    ...
    checkpoint = torch.load(args.output_dir + 'bert_crf.ckpt', map_location = device)
