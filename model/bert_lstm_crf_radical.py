...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, radical_ids, label_id):
        self.input_ids = input_ids  # 字的 ID
        self.input_mask = input_mask  # padding 为 0, 其他为 1 
        self.segment_ids = segment_ids  # 
        self.label_id = label_id  # 字标签的 ID
        self.radical_ids=radical_ids  # 每个字部首的 ID
...
def convert_examples_to_features(examples, tag2idx, max_seq_length, tokenizer, radical_dict, radical2idx):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for i, token in enumerate(tokens):
            ...
            if token in radical_dict:
                radical_ids.append(radical2idx[radical_dict[token]])
            else:
                radical_ids.append(0)
...
def get_pytorch_dataset(features):
    ...
    data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_radical_ids, all_label_ids)
...
#     train_examples, label_list, max_seq_length, tokenizer, radical_dict, radical2idx)
...
class BERT_CRF_NER(nn.Module):
    ...
    def __init__(self, bert_model, tag2idx, max_seq_length, hidden_dim, add_radical_emb, device):
        ...
        self.add_radical_emb = add_radical_emb
        self.word_embeds = nn.Embedding(num_radical + 1, self.add_radical_emb)
        self.device=device
        ...
        if self.add_radical_emb:
            self.lstm = nn.LSTM(self.embedding_dim + self.add_radical_emb, hidden_dim // 2, num_layers = 1, bidirectional = True, batch_first = True)
        else:
    ...
    ## 此处修改 logits
    def _get_bert_features(self, input_ids, segment_ids, input_mask, radical_ids):
        '''
        ...
        ## 添加部首 embedding
        if self.add_radical_emb:
            radical_emb = self._get_radical_features(radical_ids)
            bert_seq_out = torch.cat((bert_seq_out, radical_emb), dim = -1)
    ...
    def _get_radical_features(self, radical_ids):
        batch_size = radical_ids.shape[0]
        embeds = self.word_embeds(radical_ids).view(batch_size, self.max_seq_length, -1)
    ...
    def neg_log_likelihood(self, input_ids, segment_ids, input_mask, radical_ids, label_ids):
        bert_feats = self._get_bert_features(input_ids, segment_ids, input_mask, radical_ids)
    ...
    def forward(self, input_ids, segment_ids, input_mask, radical_ids):
...
parser.add_argument("--radical_emb",
                    default=20,
                    type=int,
                    help="radical embedding dim.")
...
radical_dict = joblib.load('radical.pkl')
num_radical = len(radical_dict)
radical2idx = dict([val, i] for i, val in enumerate(radical_dict.values(), 1))

# radical_dim = 20
# word_embeds = nn.Embedding(num_radical + 1, radical_dim)

# embeds = word_embeds(radical_ids)
...
if args.data_dir == 'data/ccks2018/':
    ...
    train_features = convert_examples_to_features(train_examples, tag2idx, args.max_seq_length, tokenizer, radical_dict, radical2idx)
    train_data = get_pytorch_dataset(train_features)


    dev_features = convert_examples_to_features(dev_examples, tag2idx, args.max_seq_length, tokenizer, radical_dict, radical2idx)
    dev_data = get_pytorch_dataset(dev_features)


    test_examples = processor.get_test_examples(args.data_dir)
    logger.info("number of test examples: {}".format(len(test_examples)))
    test_features = convert_examples_to_features(test_examples, tag2idx, args.max_seq_length, tokenizer, radical_dict, radical2idx)
...
# if args.do_train:
#     model = BERT_CRF_NER(bert_model, tag2idx, args.max_seq_length, args.train_batch_size, args.hidden_dim, args.radical_emb, device)

# if args.do_eval:
#     model = BERT_CRF_NER(bert_model, tag2idx, args.max_seq_length, args.eval_batch_size, args.hidden_dim, args.radical_emb, device)

model = BERT_CRF_NER(bert_model, tag2idx, args.max_seq_length, args.hidden_dim, args.radical_emb, device)
...
def evaluate(model, eval_data, out_file):
    ...
    for input_ids, input_mask, segment_ids, radical_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
        input_ids = input_ids.to(device)
        input_mask = input_mask.to(device)
        segment_ids = segment_ids.to(device)
        radical_ids = radical_ids.to(device)
        label_ids = label_ids.to(device)
        
        with torch.no_grad():
            _, predicted_ids = model(input_ids, segment_ids, input_mask, radical_ids)    # bert + crf
...
if args.do_train:
    ...
    for num_epoch in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
            ...
            input_ids, input_mask, segment_ids, radical_ids, label_ids = batch
            
            # loss = model(input_ids, segment_ids, input_mask, label_ids)
            loss = model.neg_log_likelihood(input_ids, segment_ids, input_mask, radical_ids, label_ids)
