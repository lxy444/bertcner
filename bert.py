...
import os
os.environ["CUDA_VISIBLE_DEVICES"] = "0"  # 本程序其实可以利用多 GPU 并行
...
class NerProcessor(DataProcessor):
    ...
    def get_labels(self):
        chunk_tags = ['[PAD]', 'O', 'B-anatomy', 'I-anatomy', 'E-anatomy', 'S-anatomy', 
...
# num_labels = len(label_list) + 1    # 为什么要加一
...
# encoded_layers, i = emb_model(input_ids, segment_ids)
...
parser.add_argument('--data_dir', default = 'data/ccks2018/')
        ...
        for i,mask in enumerate(input_mask):
            ...
            for j,m in enumerate(mask):
                ...
                if m and idx2tag[label_ids[i][j]] != "X" and idx2tag[label_ids[i][j]] != "[SEP]":
                    ...
                    line = x_words[j] + ' ' + idx2tag[label_ids[i][j]] + ' ' + idx2tag[logits[i][j]]
                    tag_res.write(line + '\n')
                else:
                    temp_1.pop()
                    temp_2.pop()
...
if args.do_train:
    ...
    model_config = {"bert_model":args.bert_model,"do_lower":args.do_lower_case,"max_seq_length":args.max_seq_length,"num_labels":len(label_list),"idx2tag":idx2tag}
